...
print(paste("Finished merging tissue",1))
exp_all=data.frame(fread(input = exp_files[1], header = T), check.names = F,stringsAsFactors = F)
colnames(exp_all)[-1] = paste(colnames(exp_all)[-1],tissue_names[1], sep = " - ")
for(i in 2:length(exp_files)){
  ...
  colnames(exp_t)[-1] = paste(colnames(exp_t)[-1],tissue_names[i], sep = " - ")
  
  # Merge with other tissues
  exp_all = merge(x = exp_all, y = exp_t, by="gene_id", all = TRUE)
  
  print(paste("Finished merging tissue",i))
}
...
fwrite(x = data.table(exp_all,keep.rownames = T),
       file = paste0(data.dir,"all_tissues.v8.EUR.normalized_expression_merged.txt"), 
       append = F, quote = F, sep = '\t', row.names = F, col.names = T)
print("Finished writting merged file")
