...
make_snp_sets <- function(input_dir, output_dir){
  ...
  read_csv(input_dir) %>% 
    ...
    mutate(set = case_when(exp_type == "normalized_and_residualized_expression_homogeneous" ~ "HOM",
                           exp_type == "normalized_and_residualized_expression_heterogeneous" ~ "HET",
                           exp_type == "normalized_and_residualized_expression" ~ "TBT")) %>% 
    select(set, SNP) %>% 
...
get_tested_snps <- function(MatrixEQTL_dir, output_dir){
  ...
  het_tbt <- tribble(~SNP)
  for (f in list.files(path = MatrixEQTL_dir, pattern = "normalized_and_residualized_expression_heterogeneous.all_pairs.txt", full.names = T)){
    
    print(f)
    cat(paste0("nrows: ",nrow(het_tbt),"\n"))
    
    t <- read_tsv(f, col_types = cols()) %>% select(SNP) %>% distinct
  ...
  hom <- read_tsv(paste0(MatrixEQTL_dir,"AverageTissue.v8.EUR.normalized_and_residualized_expression_homogeneous.all_pairs.txt")) %>% 
...
combine_soi_tested <- function(soi_dir, tested_dir, output_dir){
  ...
  tested <- read_csv(tested_dir) # set (HOM, HET_TBT), SNP
  soi <- read_csv(soi_dir) # set (HOM, HET, TBT), SNP
...
plot_match_stats <- function(unmatched_dir, matched_dir, maf_dir, plots_save_dir){
  ...
  matched <- read_csv(matched_dir) #set, IS_SOI, SNP, MAF_avg
...
build_cont_table_by_intersect <- function(input_dir, genomic_features_file_dir, output_dir, output_intervals_dir){
  ...
  # ~200 MBs
  snps_loc <- read_tsv("/u/project/zaitlenlab/bballiu/FastGxE/data/GTEx_v8/MatrixEQTL_input/GTEx_v8_WGS_838Indiv_Analysis_Freeze_EUR_SNPs_1prcMAF_snpsloc.txt") %>% #snp (rsIDs), chr, pos
