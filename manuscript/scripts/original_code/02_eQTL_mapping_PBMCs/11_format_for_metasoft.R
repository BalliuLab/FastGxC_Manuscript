...
context_index = as.numeric(args[6])
...
get_metasoft_files <- function(all_files, context, outdir, exp_suffix){
  ...
  cur_files = all_files[grepl(paste0("/", context), all_files)]
...
print(paste0("computing file for context ", contexts_vec[context_index]))
get_metasoft_files(all_files, contexts_vec[context_index], outdir, exp_suffix)
