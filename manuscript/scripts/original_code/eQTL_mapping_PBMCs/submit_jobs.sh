...
if ([ $JobType -eq 1 ]); then
    ...
    file="${work_dir}/scripts/eQTL_mapping/01_residualize_expression_for_covariates.R"
    for cohort in "CLUES_ASN" "CLUES_EUR" "OneK1K"; do
        ...
        for i in $(seq 1 $n_contexts); do
            qsub -N residualize_cov.$i -o ${work_dir}/logfiles/${cohort}_resexp.${i}.o -e ${work_dir}/logfiles/${cohort}_resexp.${i}.e -l h_data=16G,h_rt=12:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType $file $i $cohort $work_dir $pseudobulk $input_dir $data_dir $cov_dir
        done
    done
fi
...
#    for cohort in "CLUES_ASN" "CLUES_EUR" "OneK1K"; do
#        file="${work_dir}/scripts/eQTL_mapping/02_merge_expression_files.R"
#        qsub -N ${cohort}_merge -o ${work_dir}/logfiles/${cohort}_merge.o -e ${work_dir}/logfiles/${cohort}_merge.e -l h_data=64G,h_rt=12:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $file $work_dir $cohort
#    done
#fi
...
if ([ $JobType -eq 2 ]); then
    ...
    file="${work_dir}/scripts/eQTL_mapping/03_decompose_expression.R"
    functions_file="${work_dir}scripts/eQTL_mapping/00_functions.R"
    for cohort in "CLUES_ASN" "CLUES_EUR" "OneK1K"; do
        data_dir="${work_dir}data/${cohort}/MatrixEQTL_input/"
        SNP_file_name="${work_dir}data/${cohort}/MatrixEQTL_input/${cohort}_5prcMAF_genotypes.txt"
        qsub -N ${cohort}_decomp -o ${work_dir}/logfiles/${cohort}_decomp.o -e ${work_dir}/logfiles/${cohort}_decomp.e -l h_data=80G,h_rt=12:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType $file $work_dir $cohort $pseudobulk $data_dir $SNP_file_name $functions_file
    done
fi
...
if ([ $JobType -eq 3 ]); then
    ...
    file="${work_dir}/scripts/eQTL_mapping/05_remove_duplicated_snps.R"
    ...
    for cohort in "CLUES_ASN" "CLUES_EUR" "OneK1K"; do
        ...
        qsub -N ${cohort}_remove_dup_snps -o ${work_dir}/logfiles/${cohort}_remove_dup_snps.o -e ${work_dir}/logfiles/${cohort}_remove_dup_snps.e -l h_data=164G,h_rt=4:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType $file $work_dir $cohort $SNP_file_name $snps_location_file_name $outdir
    done
fi
...
if ([ $JobType -eq 4 ]); then
    ...
    file="${work_dir}/scripts/eQTL_mapping/06_compute_MAF_per_tissue.R"

    for cohort in "CLUES_ASN" "CLUES_EUR" "OneK1K"; do
        exp_suffix=".${cohort}.${pseudobulk}_norm_res_exp"
        exp_file_path="${work_dir}/data/${cohort}/MatrixEQTL_input/"
        SNP_file_name="${work_dir}/data/${cohort}/MatrixEQTL_input/${cohort}_5prcMAF_genotypes.txt"
        qsub -N ${cohort}_MAF -o ${work_dir}/logfiles/${cohort}_MAF.o -e ${work_dir}/logfiles/${cohort}_MAF.e -l h_data=80G,h_rt=12:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType $file $work_dir $cohort $exp_suffix $exp_file_path $SNP_file_name
    done
fi
...
if ([ $JobType -eq 5 ]); then
    ...
    outdir="${work_dir}/results/eQTL_mapping/sc_MatrixEQTL/"

    file="${work_dir}/scripts/eQTL_mapping/07_run_MatrixEQTL_by_context.R"
    for cohort in "CLUES_ASN" "CLUES_EUR" "OneK1K"; do
        n_contexts=$(ls ${scratch_dir}/${cohort}/expression_matrices/*expression.txt | wc -l)
        data_dir="${work_dir}data/${cohort}/MatrixEQTL_input/"
...
if ([ $JobType -eq 8 ]); then
    ...
    input_dir="/u/project/bballiu/bballiu/FastGxC/results/eQTL_mapping/sc_MatrixEQTL/"
    output_dir="/u/project/bballiu/bballiu/FastGxC/results/eQTL_mapping/sc_TreeQTL/"
    for cohort in "OneK1K" "CLUES_ASN" "CLUES_EUR"; do
        # Number of SNPs tested per gene for full and het
        nSNPs=1
        cd ${work_dir}/results/eQTL_mapping/sc_MatrixEQTL/
        ...
        cat tmp_MatrixEQTL_files | while read MatrixEQTL_file ; do
            ...
            qsub -N nrTests.${cohort}_${context_name}.SNPs -o ${work_dir}/logfiles/${cohort}_nrTests.${context_name}.SNPs.o -e ${work_dir}/logfiles/${cohort}_nrTests.${context_name}.SNPs.e -l h_data=16G,h_rt=12:00:00,highp $work_dir/scripts/eQTL_mapping/08_get_ntests.sh $context_name $nSNPs $cohort $input_dir $output_dir
        done
        ...
        context_name="AverageContext"
            for nSNPs in 3; do
            qsub -N nrTests.${cohort}_${context_name}.${nSNPs} -o ${work_dir}/logfiles/nrTests.${cohort}_${context_name}.${nSNPs}.o -e ${work_dir}/logfiles/nrTests.${cohort}_${context_name}.${nSNPs}.e -l h_data=16G,h_rt=12:00:00,highp $work_dir/scripts/eQTL_mapping/08_get_ntests.sh $context_name $nSNPs $cohort $input_dir $output_dir 
        done
    done
...
if ([ $JobType -eq 9 ]); then
    ...
    qsub -N FilterQTLs.${cohort}_${file} -o ${work_dir}/logfiles/FilterQTLs.${cohort}_${file}.o -e ${work_dir}/logfiles/FilterQTLs.${file}.e -l h_data=16G,h_rt=12:00:00,highp $work_dir/scripts/eQTL_mapping/09_filter_matrixEQTL_pvalue.sh ${file} $cohort $input_dir $output_dir
    done
...
file="${work_dir}/scripts/eQTL_mapping/10_run_TreeQTL.R"
functions_file="${work_dir}/scripts/eQTL_mapping/00_functions.R"
num_contexts=8
metasoft=0
m_eqtl_out_dir="${work_dir}/results/eQTL_mapping/sc_MatrixEQTL/"
treeQTL_dir="${work_dir}/results/eQTL_mapping/sc_TreeQTL/"
for cohort in "OneK1K" "CLUES_ASN" "CLUES_EUR"; do
    for exp_scale in $(seq 1 3); do
    qsub -N treeQTL.${cohort}_${exp_scale} -o ${work_dir}/logfiles/treeQTL.${cohort}_${exp_scale}.o -e ${work_dir}/logfiles/treeQTL.${cohort}_${exp_scale}.e -l h_data=80G,h_rt=20:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType $file $exp_scale $cohort $num_contexts $work_dir $functions_file $m_eqtl_out_dir $treeQTL_dir $metasoft
    done
done
fi
...
file="${work_dir}/scripts/eQTL_mapping/11_format_for_metasoft.R"
input_dir="${work_dir}/results/eQTL_mapping/sc_MatrixEQTL/"
functions_file="${work_dir}/scripts/eQTL_mapping/00_functions.R"

    for cohorts in "CLUES_ASN.CLUES_EUR.OneK1K" "CLUES_ASN.CLUES_EUR";
    do
        outdir="${work_dir}/results/eQTL_mapping/${cohorts}_Metasoft/"
        mkdir -p $outdir
        for exp_scale in $(seq 1 2); do #"CLUES_ASN" "CLUES_EUR"; do
            ...
            for i in $(seq 1 $n_contexts); do
                qsub -N Metasoft_prep_${exp_scale}.${i} -o ${work_dir}/logfiles/Metasoft_prep_${exp_scale}.${i}.o -e ${work_dir}/logfiles/Metasoft_prep_${exp_scale}.${i}.e -l h_data=80G,h_rt=20:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType $file $input_dir $exp_scale $cohorts $outdir $functions_file $i
            done
        done
    done
fi
...
for cohorts in "CLUES_ASN.CLUES_EUR.OneK1K" "CLUES_ASN.CLUES_EUR";
do
    metasoft_files_dir="${work_dir}/results/eQTL_mapping/${cohorts}_Metasoft/"
    cd $metasoft_files_dir
    rm $metasoft_files_dir/output.*.tsv

        for formatted_metasoft_file in *.tsv
        do
                output="/u/project/bballiu/bballiu/FastGxC/results/eQTL_mapping/${cohorts}_Metasoft/output.${formatted_metasoft_file}"
                qsub -N run_Metasoft -o ${work_dir}/logfiles/run_Metasoft.o -e ${work_dir}/logfiles/run_Metasoft.e -l h_data=80G,h_rt=20:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType ${metasoft_file} ${metasoft_files_dir}$formatted_metasoft_file $pvalue_table $output
        done
done
...
file="${work_dir}/scripts/eQTL_mapping/13_format_metasoft_treeQTL.R"

for cohorts in "CLUES_ASN.CLUES_EUR.OneK1K" "CLUES_ASN.CLUES_EUR";
do
    workdir="${work_dir}/results/eQTL_mapping/${cohorts}_Metasoft/"
    num_files=$(ls $workdir | grep "output" | wc -l)
    for i in $(seq 1 $num_files); do
        qsub -N format_Metasoft -o ${work_dir}/logfiles/format_Metasoft_${cohorts}.$i.o -e ${work_dir}/logfiles/format_Metasoft_${cohorts}.$i.e -l h_data=64G,h_rt=5:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType $file $workdir $i
    done
done
...
do
    ...
    do
        m_eqtl_out_dir="${work_dir}/results/eQTL_mapping/${dir}/${subdir}/"
        treeQTL_dir="${work_dir}/results/eQTL_mapping/${dir}/${subdir}/"
        for exp_scale in $(seq 1 3); do
            cohort=""
            qsub -N treeQTL.${dir}_${subdir}_${exp_scale} -o ${work_dir}/logfiles/treeQTL.${dir}_${subdir}_${exp_scale}.o -e ${work_dir}/logfiles/treeQTL.${dir}_${subdir}_${exp_scale}.e -l h_data=80G,h_rt=10:00:00,highp $work_dir/scripts/eQTL_mapping/submit_job_lena.sh $JobType $file $exp_scale "" $num_contexts $work_dir $functions_file $m_eqtl_out_dir $treeQTL_dir $metasoft
        done
    done
done
fi
